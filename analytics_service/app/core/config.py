...
class ZeroSecretsManager:
    ...
    async def get_config(self, service_name: str) -> Dict[str, Any]:
        ...
        Args:
            service_name: Name of the service (e.g., 'analytics_service')
...
class AnalyticsConfig:
    """
    Analytics Service configuration manager.
    Handles all configuration for the analytics service.
    """
    ...
    async def get_database_url(self) -> str:
        """Get the database connection URL (shared across services)."""
    ...
    async def get_auth_service_url(self) -> str:
        """Get the auth service URL for token validation."""
        return await self.secrets_manager.get_secret("AUTH_SERVICE_URL") or "http://localhost:8000"
    ...
    async def get_analytics_cache_ttl(self) -> int:
        """Get analytics cache TTL in seconds."""
        ttl = await self.secrets_manager.get_secret("ANALYTICS_CACHE_TTL")
        return int(ttl) if ttl else 300  # 5 minutes default
    
    async def get_rate_limit_config(self) -> Dict[str, int]:
        """Get rate limiting configuration."""
        return {
            "analytics_requests": int(await self.secrets_manager.get_secret("RATE_LIMIT_ANALYTICS") or "100"),
            "window_minutes": int(await self.secrets_manager.get_secret("RATE_LIMIT_WINDOW") or "15")
        }
    
    async def get_event_retention_days(self) -> int:
        """Get event retention period in days."""
        days = await self.secrets_manager.get_secret("EVENT_RETENTION_DAYS")
        return int(days) if days else 90  # 3 months default
