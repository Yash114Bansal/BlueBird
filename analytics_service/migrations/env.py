...
try:
    ...
    logger.info("Successfully imported analytics models")
...
# Configure Alembic to only manage analytics tables
def include_object(object, name, type_, reflected, compare_to):
    """Filter objects to only include analytics tables."""
    if type_ == "table":
        # Only include tables that are defined in our analytics models
        # Get table names from our Base metadata
        analytics_table_names = set(target_metadata.tables.keys())
        return name in analytics_table_names
    return True
