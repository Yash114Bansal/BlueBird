...
version_table = "alembic_version_bookings"
# Set up logging
logging.basicConfig(
    level=logging.INFO,
...
try:
    ...
    logger.info("Successfully imported booking models")
...
# Configure Alembic to only manage bookings tables
def include_object(object, name, type_, reflected, compare_to):
    """Filter objects to only include bookings tables."""
    if type_ == "table":
        # Only include tables that are defined in our booking models
        # Get table names from our Base metadata
        booking_table_names = set(target_metadata.tables.keys())
        return name in booking_table_names
    return True
