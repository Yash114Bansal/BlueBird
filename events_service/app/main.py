"""
Main FastAPI application for Events Service.
Entry point for the events microservice.
"""
...
app = FastAPI(
    title="Evently Events Service",
    description="Events management microservice for Evently platform",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
    lifespan=lifespan
...
async def root():
    ...
    """
    return {
        "service": "Evently Events Service",
        "version": "1.0.0",
        "description": "Events management microservice",
        "docs": "/docs",
        "health": "/health"
    }
...
async def service_info():
    ...
    return {
        ...
        "description": "Events management microservice for Evently platform",
        "endpoints": {
            "user_events": "/v1/events",
            "admin_events": "/v1/admin/events",
            "health": "/health",
            "docs": "/docs"
        },
        "features": [
            "User event browsing (JWT auth required)",
            "Admin event management (admin JWT auth required)",
            "Event listing with caching",
            "Redis caching for performance",
            "JWT authentication with role-based access",
            "Separate user and admin API routes"
        ]
    }
